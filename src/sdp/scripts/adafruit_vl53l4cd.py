...
class VL53L4CD:
    ...
    def data_status(self):
        """Returns data status."""
        status = [255, 255, 255, 5, 2, 4,
        1, 7, 3, 0, 255, 255,
        9, 13, 255, 255, 255, 255,
        10, 6, 255, 255, 11, 12]
        temp = self._read_register(_VL53L4CD_RESULT_RANGE_STATUS)
        temp_8 = struct.unpack(">B", temp)[0] & 0x1f
        if temp_8 < 24:
            return status[temp_8]
        return temp_8

    @property
    def xtalk_value(self):
        """Returns data status."""
        return self._read_register(_VL53L4CD_XTALK_PLANE_OFFSET_KCPS)

    @property
    def offset_value(self):
        return self._read_register(_VL53L4CD_RANGE_OFFSET_MM)

    def offset_calibration(self, target_distance, samples):
        self._write_register(_VL53L4CD_RANGE_OFFSET_MM, struct.pack(">H", 0))
        self._write_register(_VL53L4CD_INNER_OFFSET_MM, struct.pack(">H", 0))
        self._write_register(_VL53L4CD_OUTER_OFFSET_MM, struct.pack(">H", 0))
        self._write_register(_VL53L4CD_XTALK_PLANE_OFFSET_KCPS, struct.pack(">H", 0))
        average_distance = 0
        self.start_ranging()
        # warmup
        for i in range(10):
        ...
        for i in range(samples):
            average_distance += self.get_reading() * 10
        self.stop_ranging()

        # calculate offset
        average_distance /= samples
        offset = target_distance - average_distance
        tmp_offset = int(offset * 4)
        print(offset, target_distance, average_distance)
        print(tmp_offset)
        packed = struct.pack(">h", tmp_offset)
        self._write_register(_VL53L4CD_RANGE_OFFSET_MM, packed)
        return tmp_offset, offset

    def xtalk_calibration(self, target_distance, samples):
        average_distance = 0
        average_spad = 0
        average_signal = 0
        self._write_register(_VL53L4CD_XTALK_PLANE_OFFSET_KCPS, struct.pack(">H", 0))
        ...
        for i in range(samples):
            average_distance += self.get_reading() * 10
            average_spad += self.get_spad()
            average_signal += self.get_signal()
        self.stop_ranging()

        # calculate offset
        average_distance /= samples
        average_spad /= samples
        average_signal /= samples
        xtalk = 512 * (average_signal * (1 - average_distance /target_distance)) / average_spad
        xtalk = int((xtalk * 1000) / 512 )
        packed = struct.pack(">h", xtalk)
        self._write_register(_VL53L4CD_XTALK_PLANE_OFFSET_KCPS, packed)
        return xtalk

    def get_reading(self, timeout=5000):
        j = 0
        while True:
            if self.data_ready:
                break
    ...
    def get_spad(self):
        spad = self._read_register(_VL53L4CD_RESULT_SPAD_NB, 2)
        spad = struct.unpack(">H", spad)[0]
        return spad

    def get_signal(self):
        signal = self._read_register(_VL53L4CD_RESULT_SIGNAL_RATE, 2)
        signal = struct.unpack(">H", signal)[0]
        return signal
